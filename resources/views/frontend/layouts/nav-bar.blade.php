    ...
    <div class="container-fluid m-pad">
        <div class="menu-header">
            <div class="dsk-logo"><a class="nav-brand" href="/">
                    <img src="@if (isset($_settings['logo'])) {{ \App\Facades\ViewHelperFacade::getImagePath('site_configuration', $_settings['logo']) }} @endif"
                        alt="Logo" class="mega-logo">
                </a></div>
            <div class="custom-nav" role="navigation">
                <ul class="nav-list">
                    ...
                    @if (isset($_menus['top-bar-menu']))
                        @forelse($_menus['top-bar-menu']['pages'] as $page)
                            @if ($page->page_type == 'link')
                                <li><a href="{{ url($page->link) }}" class="menu-links"
                                        @if ($page->open_in == 'new') target="_blank" @endif>{{ $page->title }}</a>
                                </li>
                            @else
                                <li><a href="{{ route('page.menu', $page->slug) }}" class="menu-links"
                                        @if ($page->open_in == 'new') target="_blank" @endif>{{ $page->title }}</a>
                                </li>
                            @endif
                        @empty
                    ...
                    <li>
                        <a href="{{ route('request-a-quote') }}" class="btn-br bg-btn3 btshad-b2 lnk">Request A Quote
            ...
            <div class="mobile-menu2">
                <ul class="mob-nav2">
                    <li class="navm-"> <a class="toggle hc-nav-trigger hc-nav-1" href="#" role="button"
