                            ...
                            <div class="table-responsive">
                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                    ...
                                    <tr>
                                        <th colspan="8">
                                            <div class="widget-main">
                                                ...
                                                <button type="button" class="btn btn-info btn-xs" id="bulk-action-submit-btn">
                                    ...
                                    <tr>
                                        ...
                                        <th>{!! Form::text('filter_type', null, ['class' => 'form-control']) !!}</th>
                                        <th>{!! Form::text('filter_hint', null, ['class' => 'form-control']) !!}</th>
                                        <th>{!! Form::date('filter_created_at', null, ['class' => 'form-control']) !!}</th>
                                        <th>{!! Form::date('filter_updated_at', null, ['class' => 'form-control']) !!}</th>
                                       {{-- <th>{!! Form::date('filter_created_at', null, ['class' => 'form-control']) !!}</th>
                                        <th>{!! Form::date('filter_updated_at', null, ['class' => 'form-control']) !!}</th>--}}
                                        <th>
                                            {!! Form::select('filter_status', ['all' => 'All', 'active' => 'Active', 'inactive' => 'Inactive'], null, ['class' => 'form-control']) !!}</th>
                                        <th>
                                    ...
                                    @if ($data['rows']->count() > 0)
                                        @foreach($data['rows'] as $row)
                                            ...
                                            <tr>
                                                ...
                                                <td class="hidden-480">
                                                    @if ($row->ad_type == 'script' )
                                                        <span class="label  label-lg label-grey">
                                                            <i class="icon-code bigger-120">&nbsp   &nbsp</i>
                                                            {{ $row->ad_type }}
                                                        </span>
                                                    @else
                                                        <span class="label label-lg label-primary">
                                                            <i class="icon-picture bigger-120">&nbsp</i>
                                                            {{ $row->ad_type }}
                                                        </span>
                                                    @endif
                                                </td>
