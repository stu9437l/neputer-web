...
use App\Http\Requests\Technology\AddFormValidation;
use App\Http\Requests\Technology\EditFormValidation;
...
{
    protected $model;
    protected $base_route = 'admin.technologies';
    protected $view_path = 'admin.technologies';
    ...
    public function __construct()
    {
        ...
        $this->folder = config('myPath.assets.image_panel_folder.technology');
    ...
    public function index(Request $request)
    {
        ...
        $data['rows'] = $this->model
            ->select(
                'id', 'created_at', 'updated_at', 'title', 'icon', 'description', 'status'
            )
            ->where(function ($query) use ($request) {

                if ($request->has('filter_title') && $request->get('filter_title')) {
                    $query->where('technologies.title', 'like', '%' . $request->get('filter_title') . '%');
                }

                if ($request->has('filter_icon') && $request->get('filter_icon')) {
                    $query->where('technologies.icon', 'like', '%' . $request->get('filter_icon') . '%');
                }


                if ($request->has('filter_created_at') && $request->get('filter_created_at')) {
                    $query->where('technologies.created_at', 'like', $request->get('filter_created_at') . '%');
                }

                if ($request->has('filter_updated_at') && $request->get('filter_updated_at')) {
                    $query->where('technologies.updated_at', 'like', $request->get('filter_updated_at') . '%');
                }

                if ($request->has('filter_status') && $request->get('filter_status') && $request->get('filter_status') !== 'all') {
                    $query->where('technologies.status', $request->get('filter_status') == 'active' ? 1 : 0);
                }
    ...
    public function store(AddFormValidation $request)
    {
        ...
        $technologies = $this->model->create([
            'title' => $request->get('title'),
            'icon' => $this->image_name,
    ...
    public function update(EditFormValidation $request, $id)
    {
        ...
        $this->image_name = $data['row']->icon;
        parent::uploadImage($request, 'update', $data['row']->icon);
